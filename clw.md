...
Как сказано выше, **ps -Aco pid,pcpu,rss,comm** покажет список запущенных
программ. Отсортируем его по загрузке процессора: это второе поле, оно числовое.
Используем **sort** вот так: **ps -Aco pid,pcpu,rss,comm | sort -k2n**. Третье
поле — объем занимаемой памяти, попробуем отсортировать по нему: **ps -Aco
pid,pcpu,rss,comm | sort -k3n**. Самые "жирные" оказались внизу, ведь сортировка
ведется по возрастанию. Выберем top 10 самых прожорливых программ и отсортируем
их так, чтобы самая прожорливая была сверху: **ps -Aco pid,pcpu,rss,comm | sort
-k3nr | head -10**.
